...
export function buildRenewalResearchPrompt(
  ...
  const lines = [
    ...
    '',
    'BUSINESS SCOUTING & RESEARCH (REQUIRED BEFORE WRITING OUTREACH):',
    '- Do not guess a merchant\'s business model, products, services, customers, or likely uses of capital from the business name alone. A name like "Summit Logistics" or "Bella Salon" proves nothing until a matching source confirms it.',
    '- Every operational fact in the profile must come from a source that you matched to this exact merchant. If no source can be matched, leave businessType, products, services, customerType, businessModel, and workingCapitalUses empty and set exactBusinessVerified to false; never fill them from what the name implies.',
    '',
    '1. IDENTIFY THE EXACT BUSINESS',
    '- Match the merchant using as many of the following as available: legal business name, account name, DBA/business name, the universal business locator, full business address, website, phone number, and merchant/contact or owner name.',
    ...
    '- Research the official website first: open the supplied website (or the domain from the merchant email) and read its home, about, services/products, and contact pages.',
    '- Research public business profiles: Google Maps, Google Business Profile, Yelp, BBB, LinkedIn, Facebook/Instagram business pages, state business registrations, and reputable industry directories when available.',
    ...
    '- Describe what the business does only in terms a matched source actually states. Do not upgrade a vague listing category into specific products, services, or customers.',
    '',
    ...
    '- Use the category examples only as a guide to the level of specificity expected; the uses you return must follow from what the matched sources show this merchant doing, not from the category its name suggests.',
    '',
...
export function buildRenewalGenerationPrompt(context: RenewalMerchantContext): string {
  ...
  const repCompany = context.representative.company.trim();
  const smsIntro = repName ? `This is ${repName}${repCompany ? ` with ${repCompany}` : ''}.` : '';
  ...
  return [
    ...
    '- Use researched facts only when exactBusinessVerified is true; otherwise rely only on supplied merchant and funding fields and do not describe what the business does, sells, or serves.',
    '- Never infer what the business does from its name. Strictly avoid phrases such as "Based on your business name", "Judging from the company name", "Your name suggests", "Given the name", "As the name implies", or any wording that reveals the business type was guessed from the name.',
    '- When you reference the business, write as someone who looked at the actual operation (for example, "Based on what I saw about the shop"), and only when exactBusinessVerified is true.',
    ...
    ...(smsIntro
      ? [
          `- SMS introduction: the SMS must open with "${greeting}, ${smsIntro}" exactly, using representative.name and representative.company as given, before any mention of the lender, funding, or the business. Do not write "it is", "my name is", or "I am" in place of "This is". The email does not need this introduction because the signature block is appended.`,
        ]
      : []),
    ...
    `- Greeting: the first line is exactly "${greeting}," per TONE AND COURTESY.`,
