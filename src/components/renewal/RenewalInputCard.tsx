...
const REQUIRED_FIELDS: Array<{
  ...
  {
    key: 'businessLocator',
    label: 'Business Website / Google Maps / Address',
    placeholder: 'Website, Maps link, or street, city, state, ZIP',
    wide: true,
  },
...
const FUNDING_DETAIL_FIELDS: Array<{
  ...
  { key: 'renewalEligibilityDate', label: 'Renewal date', manual: true },
  { key: 'existingPositions', label: 'Existing positions', manual: true },
  { key: 'existingOutstandingOffer', label: 'Outstanding offer', manual: true },
  { key: 'possibleLineOfCredit', label: 'LOC amount', manual: true },
    ...
    <Card title="Renewal details" icon={<SparklesIcon className="size-3.5" />}>
      <div className="space-y-3">
        <p className="text-xs text-content-secondary">
          Enter the five essentials. SideRep identifies the business, researches it, and uses
          optional funding details when available.
        </p>
        ...
        <details className="rounded-lg border border-edge bg-surface-2/40 p-3">
          ...
          <div className="mt-3 space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              {FUNDING_DETAIL_FIELDS.map((field) => (
                <div key={field.key} className={field.wide ? 'sm:col-span-2' : undefined}>
                  <Field
                    label={field.label}
                    hint={field.manual ? 'Optional; not saved between sessions.' : undefined}
                  >
                    <Input
                      aria-label={field.label}
                      type={field.type}
            ...
            >
              <Select
                ...
                onChange={(event) =>
                  renewal.setOutreachType(event.target.value as RenewalOutreachType)
                }
              >
